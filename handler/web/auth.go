...
func (a *authWeb) Login(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		...
		loginBody := path.Join("views", "auth", "login.html")
		var templ = template.Must(template.ParseFS(a.embed, loginBody, gHeader))
...
func (a *authWeb) Register(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		gHeader := path.Join("views", "general", "header.html")
		regisBody := path.Join("views", "auth", "register.html")
		var templ = template.Must(template.ParseFS(a.embed, regisBody, gHeader))
		var err = templ.Execute(w, nil)
		if err != nil {
			panic(err) //tes
		}
