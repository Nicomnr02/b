...
func (d *dashboardWeb) Dashboard(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		dashBody := path.Join("views", "main", "dashboard.html")
		var templ = template.Must(template.ParseFS(d.embed, dashBody, gHeader)).Funcs(funcMap)

		var err = templ.Execute(w, dataTemplate["categories"])
		if err != nil {
			panic(err) //tes
		}
