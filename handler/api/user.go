...
func (u *userAPI) Login(w http.ResponseWriter, r *http.Request) {
	...
	if validUserID, err := u.userService.Login(r.Context(), &entity.User{Email: user.Email, Password: user.Password}); err != nil {
		...
		fmt.Println("id dari login == ", validUserID)
		id := strconv.Itoa(validUserID)
		...
		json.NewEncoder(w).Encode(map[string]interface{}{"user_id": validUserID, "message": "login success"})
	}
...
func (u *userAPI) Register(w http.ResponseWriter, r *http.Request) {
	...
	if newUser, err := u.userService.Register(ctx, &entity.User{Fullname: user.Fullname, Email: user.Email, Password: user.Password}); err != nil {
